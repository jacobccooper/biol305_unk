...
You must first download the data from this link and save them to your computer: [DOWNLOAD LINK](https://figshare.com/ndownloader/files/15070175).

After saving to your computer, you will read them in using a code like the following (*adjusted for YOUR machine!*):
...
```{r, eval = F}
# Mac / Linux
canines <- read_csv("~/Folder/Path/To/Measurements.csv")

# Windows
filepath <- r'C:\Users\yourname\folder\path\Measurements.csv'
canines <- read_csv(filepath)
```

```{r,message = FALSE, echo=FALSE}
canines <- read_csv("~/OneDrive - University of Nebraska/UNK/Classes/BIOL 305/Assignments/t-test/Measurements.csv")
```
...
Tuition is a major factor for most students when picking a university. You are curious whether UNK is a good deal so far as Nebraska schools are concerned; to make the comparison fair, you decide to compare the total in-state costs for all Nebraska schools offering 4-year degrees.

```{r, message=FALSE}
tuition <- read_csv("https://raw.githubusercontent.com/rfordatascience/tidytuesday/main/data/2020/2020-03-10/tuition_cost.csv") |> 
  filter(state == "Nebraska" & degree_length == "4 Year")

unk <- tuition[18,]
other_tuition <- tuition[-18,]
```
...
- Is the tuition at UNK less than the average cost of a four-year degree in Nebraska? (2 pts)

- Now, perform the above as a one-tailed test (if you did a two-tailed) or as a two-tailed test (if you did a one-tailed). Does your answer change? Does your question or hypothesis change? Explain.
...
## Homework: Two-sample means testing
...
### Question 2: The maximum airspeed velocity of a swallow

Inspired by [*Monty Python and the Holy Grail*](https://www.youtube.com/watch?v=liIlW-ovx0Y), you decide to compare the maximum airspeed velocity of African and European swallows, namely, the European Red-rumped Swallow *Cecropis rufula* African Red-rumped Swallow *Cecropis melanocrissus* to see if they differ in some way. You measure one group of each of these swallows and get the following dataset:
...
### Question 3: Heart rate and salsa
...
tuesdata <- flint_mdeq <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/main/data/2025/2025-11-04/flint_mdeq.csv')
flint_mdeq <- tuesdata |> 
  dplyr::select(-notes) |> 
  na.omit() |> 
  dplyr::select(-lead2)
...
- What is the goal of the analysis above?
...
[According to the EPA](https://www.epa.gov/ground-water-and-drinking-water/basic-information-about-lead-drinking-water), if more than 10% of tap water samples exceed the lead action level of 15 parts per billion, then further actions must be taken to mitigate and help control the lead, as well as to educate the public.

- What is the percent of samples in the `flint_mdeq` that are above the 15 parts per billion threshold?

- What is the average lead level in the town?
