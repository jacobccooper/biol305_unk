...
Now, we can use the aforementioned Tukey results to assign to groups. We do have to create this part by hand, unfortunately.

```{r}
# note first group must be EXACT MATCH to your gg_data object
sig.labels <- data.frame(Group = c("x1", "x2", "x3", "x4"),
                         # significance of groups
                         significance = c("A", "B", "B", "B"))
```

Now we can plot our data and add the labels!
