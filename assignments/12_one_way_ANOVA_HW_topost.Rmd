...
Researchers want to test a new drug when given at different dosages. They are also including a placebo. They are measuring the score on a psychological examination, and are hoping to find the most effective dose to reduce the score. Run the proper tests to determine if each group is unique and what effects they have.
...
drug |> 
  group_by(Dose) |> 
  dplyr::summarise(p_value = shapiro.test(Score)$p.value)
```
...
Colonies_data |> 
  group_by(Colony) |> 
  summarise(p.value = shapiro.test(sqrt(Minutes))$p.value)
```

```{r, echo = F}
Colonies_data |> 
  mutate(Minutes_sqrt = sqrt(Minutes)) |> 
  run_oneway(dv = "Minutes_sqrt", group = "Colony", parametric = T) ->
  a
...
vole |> 
  ...
  summarise(p.value = shapiro.test(Aggression)$p.value)
```

```{r, echo = F}
run_oneway(data = vole,
           dv = "Aggression",
           group = "Environment",
           parametric = T) ->
  a
