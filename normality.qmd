...
```{r, echo = F, eval = F}
zscore(92, mu = mean(clupea.lengths), 
...
> The mean length of *Ammodytes marinus* ($X \pm Y mm$) is no different than the mean length of *Ammodytes tobianus* ($X \pm Ymm$, $Z_{(n)} = 1.2$, $p = 0.12$).

Note that $n$ above would be replaced with your sample size for *Ammodytes tobianus,* and the X's replaced with the values of the means from your dataset and the Y's replaced with your confidence interval value corresponding to your $\alpha$.
