...
Later on in the tutorial, we will also use the `Outlier` command from the `DescTools` package. That will have to be installed and then loaded using library as well.
...
However, the library *UNKstats* has many missing basic statistical functions, including `find_mode()`. Use this method in your code to reduce clutter and make the code run faster.

```{r}
find_mode(fraps)
```

The output of this function is two lines. The first line returns the value(s) for the mode, while the second line tells us the frequency with which this value occurs.
...
There is not a built in function for standard error in *R*, but we can use `UNKstats`.
...
We can also use the function in `UNKstats`.
...
Please create an *RMarkdown* document that will render as an `.html` file. You will submit this file to show your coding and your work. Please refer to the [Introduction to *R*](https://jacobccooper.github.io/biol105_unk/intro_to_r.html) for refreshers on how to create an `.html` document in *RMarkdown*.
