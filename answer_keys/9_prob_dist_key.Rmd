...
```{r, echo = F}
a <- dbinom(x = 3, size = 50, p = 1/50)

a
```

The probability is low: 0.06.
...
```{r, echo = F}
lambda <- 557/(736000+557)*5000

lower <- dpois(x = c(0:1), lambda = lambda) |> 
  sum()

(1-lower) |> round(4)
```

Assuming they are randomly distributed, the probability should be 0.89.
...
```{r, echo = F}
a <- dbinom(x = 4, size = 30, p = 1/20)
a |> round(2)
```

No, the dice may be fair. Probability is 0.06.
...
Approximately 16%. Report as 0.16.
