...
```{r}
Z <- zscore(xbar = 85, mu = mean(minnow.length),
       sd.x = sd(minnow.length), n = length(minnow.length))
Z |> round(2)
```
...
The mean length of minnows in the Kearney Canal is less than the mean length of published minnow populations ($Z = 2.04$, $p = 0.02$).
...
4.  What is the 95% confidence interval for the mean of these data? Report as the mean +/- the confidence interval, and answer in a full sentence. Don't forget to mention the units of your answer. HINT: you may have to back-transform your data.
