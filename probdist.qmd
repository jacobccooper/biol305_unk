...
**Answers are provided at the end to help guide you, but you will be graded on whether you show your work!**
...
```{r, echo = F, eval = F}
bt <- binom.test(x = 3, # number of successes
           n = 50, # number of trials
           p = 1/50) # odds of winning
...
```{r, echo = F, eval = F}
dbinom(x = 5, size = 200, 0.03)
```

### Problem 6

Down on your luck, you decide to look for some four-leaf clovers. You notice that you seem to find 1 four-leaf clover out of every 5,000 that you check (based on [Reader's Digest article](https://www.rd.com/article/four-leaf-clover/)). What are the odds of checking 2,500 clovers and finding *any* clovers? (Round to the first decimal place with a non-zero integer).

```{r, echo = F, eval = F}
1-dpois(x = c(0), lambda = 1/2500)
```

### Problem 7

You are giving mice an experimental treatment that is designed to change their fur color. In a trial of 500 mice, 15 had their fur change color. Using this precedent, are the odds of having 8 mice have their fur change color in another trial of 500 individuals?

```{r, echo = F, eval = F}
dbinom(x = 8, size = 500, prob = 15/500)
```
...
```{r, echo = F, eval = F}
dbinom(x = c(0:2), size = 20, prob = 0.10) |> sum() -> soda_prob
...
<summary>Answers to previous questions.</summary>

1.  The probability is 0.08.
2.  The probability is 0.33.
3.  The probability is 0.06.
4.  The probability is 0.07. (NOTE: some variation depending on exact calculator).
5.  The probability is 0.16.
6.  The probability is 0.0004.
7.  The probability is 0.02.
8.  The probability is 0.90.
9.  The probability is 0.32.
