...
Here, we get a $p$ value of `r round(pchisq(sum(chi_matrix), df = 1, lower.tail = F),3)`.
...
Now we have a $\chi^2$ of `r round(sum(e_o2),2)` - which, as we will see below, is the exact same value as we get for an uncorrected `chisq.test` from *R*'s default output.
...
Here, we get $p = $`r round(chi_data$p.value,3)`, which is significant with $\alpha = 0.05$.
