...
## Homework: $\chi^2$ tests

**For all homework problems, assume $\alpha = 0.05$.

### La Selva Bird Counts

The La Selva station in Costa Rica performed Christmas Bird Counts for 23 years between 1989-2011 and used these data to analyze the status and trends of different birds within the region [@boyle_2016_4979259]. Here, we will look at their data for some of the "big groundbirds" - namely, members of the families [Tinamidae](https://en.wikipedia.org/wiki/Tinamou) and [Cracidae](https://en.wikipedia.org/wiki/Cracidae) that are isolated in the object `big_groundbirds` below.
...
1.  Is each species equally likely to be encountered across years (`NYears`)?

-   state a null and alternative hypothesis for this scenario

-   Perform the appropriate $\chi^2$ test

-   Interpret the test, stating your conclusions and listing the test statistics and the $p$ value

2.  Does each species have an equal high count (`MaxN`)?
...
### Bicycle counts: Time of day

We are going to revisit the bicycle data of @weber_fremont_2019 to examine two different temporal patterns in bridge use.
...
```{r}
# isolate data during the day on March 31
march_31 <- bicycles |> 
  filter(day(Date) == 31) |> 
  filter(hour(Date) > 7 & hour(Date) < 18)
```

1. Are bicycle counts the same for each hour in Eastbound traffic?
...
2. Are bicycle counts the same for each hour in Westbound traffic?
...
3. Are bicycle counts the same per hour for comparing eastbound to westbound traffic?
...
1. Are the counts the same per hour between dates for eastbound traffic?
