...
The following cumulative distribution plot is based on a method from [Geeks for Geeks](https://www.geeksforgeeks.org/cumulative-frequency-graph-in-r/).
...
break_points <- seq(-3,3,1)
```

```{r}
# transforming the data
data_transform = cut(x, break_points,
					right=FALSE)
...
- `c` means "contatenate", or place things together in an object.

```{r}
# lower bounds for 100-199, 200-300
# values denote counts in category
y <- c(17,24,16)

# force histogram to these values
barplot(y, 
