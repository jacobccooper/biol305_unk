...
data <- data_expanded |>
  ...
  pivot_longer(!Blocking_Variable, 
               names_to = "Explanatory_Variables", 
...
3.  Test for normality relative to the primary hypothesis *only*. Do not transform problems that tell you to ignore normality requirements.
...
6.  If there are any additional questions within the problem, be sure to address them
...
**Do not transform these data - perform as is.**
...
alone |> 
  group_by(gender) |> 
  summarise(pv = shapiro.test((days_lasted)^(1/2))$p.value)
...
penguins |> 
  group_by(species) |> 
  summarise(pv = shapiro.test(log1p(bill_len))$p.value)

penguins <- penguins |> 
  mutate(log_bill = log1p(bill_len))

aov <- run_twoway(data = penguins, dv = "log_bill", factor_a = "species", factor_b = "sex",
                  include_interaction = T, which_factor = "species:sex")

aov$anova_table
```

There is a significant effect of species ($F_{2,327} = 673.82$, $p < 0.001$) and the intraspecific variable ($F_{1,327} = 199.47$, $p < 0.001$).
...
There is no difference in means between years ($F_{1.67, 6.70} = 2.25$, $p = 0.18$).
...
res_pretrial <- run_twoway(
  data                = pretrial_anova,
  dv                  = "rate_per_100000",
  factor_a            = "pop_category",      
  factor_b            = "urbanicity",   
  include_interaction = TRUE,
  which_factor        = "pop_category:pop_category" 
)
...
Using datasets from `tidytuesdayR` ([viewable here](https://github.com/rfordatascience/tidytuesday)) or a different dataset of your choosing, create your own two-way ANOVA homework problem.
